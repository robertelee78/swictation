#!/bin/bash
# macOS ARM64 platform package build script
# Builds Rust binaries and packages them for @swictation/darwin-arm64
# Designed to run in CI/CD (GitHub Actions macos-14) or locally on Apple Silicon
set -e
...
echo "🍎 Building @swictation/darwin-arm64 platform package..."
echo ""
echo "📁 Directories:"
...
if [ "$ARCH" != "arm64" ]; then
    ...
    echo "   Build may fail or produce incorrect binaries for Apple Silicon"
fi
...
if command -v jq &> /dev/null; then
    ...
    ONNX_VERSION=$(jq -r '.libraries.onnxruntime."macos-coreml"' "$VERSIONS_FILE")
else
    ...
    ONNX_VERSION=$(grep '"macos-coreml"' "$VERSIONS_FILE" | sed 's/.*: *"\([^"]*\)".*/\1/')
fi

echo "   Distribution: $DISTRIBUTION_VERSION"
echo "   Daemon:       $DAEMON_VERSION"
echo "   UI:           $UI_VERSION"
echo "   ONNX Runtime: $ONNX_VERSION (CoreML)"
echo ""
...
echo "   Running: cargo build --release --target aarch64-apple-darwin --workspace"
...
echo "   Size: $(du -h "$DAEMON_SOURCE" | awk '{print $1}')"
echo ""
...
echo "   Running: cargo tauri build --target aarch64-apple-darwin"
cargo tauri build --target aarch64-apple-darwin

UI_SOURCE="$REPO_ROOT/tauri-ui/src-tauri/target/aarch64-apple-darwin/release/swictation-ui"
if [ ! -f "$UI_SOURCE" ]; then
    # Try alternative location (bundle)
    UI_SOURCE="$REPO_ROOT/tauri-ui/src-tauri/target/aarch64-apple-darwin/release/bundle/macos/swictation-ui.app/Contents/MacOS/swictation-ui"
    if [ ! -f "$UI_SOURCE" ]; then
        echo "❌ ERROR: UI binary not found at expected locations"
        exit 1
    fi
fi
...
echo "   Size: $(du -h "$UI_SOURCE" | awk '{print $1}')"
echo ""
...
# ============================================================================
# STEP 8: Copy ONNX Runtime Library (CoreML-enabled)
# ============================================================================
echo "📚 Step 8: Copying ONNX Runtime library (CoreML)..."

# Try to find libonnxruntime.dylib in known locations
ONNX_LIB=""
SEARCH_PATHS=(
    "$REPO_ROOT/npm-package/lib/native/libonnxruntime.dylib"
    "/usr/local/lib/libonnxruntime.dylib"
    "/opt/homebrew/lib/libonnxruntime.dylib"
    "$HOME/.local/lib/libonnxruntime.dylib"
)

for path in "${SEARCH_PATHS[@]}"; do
    if [ -f "$path" ]; then
        ONNX_LIB="$path"
        echo "   Found libonnxruntime.dylib at: $path"
        break
    fi
done

if [ -z "$ONNX_LIB" ]; then
    echo "⚠️  Warning: libonnxruntime.dylib not found in standard locations"
    echo "   The CoreML-enabled library will need to be provided separately"
    echo "   Expected locations:"
    for path in "${SEARCH_PATHS[@]}"; do
        echo "     - $path"
    done
    echo ""
    echo "   Note: ONNX Runtime 1.22.0 with CoreML is required (not 1.23.x)"
    echo "   Download from: https://github.com/microsoft/onnxruntime/releases/tag/v1.22.0"
else
    cp "$ONNX_LIB" "$PLATFORM_LIB_DIR/libonnxruntime.dylib"
    echo "✓ Copied libonnxruntime.dylib"
    echo "   Size: $(du -h "$PLATFORM_LIB_DIR/libonnxruntime.dylib" | awk '{print $1}')"

    # Verify it's for arm64
    if file "$PLATFORM_LIB_DIR/libonnxruntime.dylib" | grep -q "arm64"; then
        echo "   ✓ Verified: arm64 architecture"
    else
        echo "   ⚠️  Warning: Library may not be arm64 architecture"
    fi
fi
echo ""
...
{
    echo "# macOS ARM64 Build Checksums"
    ...
    echo ""
    echo "## Binaries"
    shasum -a 256 "$PLATFORM_BIN_DIR/swictation-daemon" | sed "s|$PLATFORM_BIN_DIR/||"
    shasum -a 256 "$PLATFORM_BIN_DIR/swictation-ui" | sed "s|$PLATFORM_BIN_DIR/||"

    if [ -f "$PLATFORM_LIB_DIR/libonnxruntime.dylib" ]; then
        echo ""
        echo "## Libraries"
        shasum -a 256 "$PLATFORM_LIB_DIR/libonnxruntime.dylib" | sed "s|$PLATFORM_LIB_DIR/||"
    fi
} > "$CHECKSUMS_FILE"
...
# Check that binaries are Mach-O arm64
DAEMON_FILE_TYPE=$(file "$PLATFORM_BIN_DIR/swictation-daemon")
UI_FILE_TYPE=$(file "$PLATFORM_BIN_DIR/swictation-ui")

if echo "$DAEMON_FILE_TYPE" | grep -q "Mach-O.*arm64"; then
    echo "✓ swictation-daemon: Mach-O arm64 (correct format)"
else
    echo "❌ ERROR: swictation-daemon is not Mach-O arm64"
    echo "   File type: $DAEMON_FILE_TYPE"
    exit 1
fi

if echo "$UI_FILE_TYPE" | grep -q "Mach-O.*arm64"; then
    echo "✓ swictation-ui: Mach-O arm64 (correct format)"
else
    echo "❌ ERROR: swictation-ui is not Mach-O arm64"
...
otool -L "$PLATFORM_BIN_DIR/swictation-daemon" | tail -n +2 | head -10 | sed 's/^/     /'

echo ""
echo "   UI dependencies:"
otool -L "$PLATFORM_BIN_DIR/swictation-ui" | tail -n +2 | head -10 | sed 's/^/     /'

# Check for CoreML/Metal frameworks
echo ""
echo "   Checking for CoreML/Metal support..."
if otool -L "$PLATFORM_BIN_DIR/swictation-daemon" | grep -q -i "coreml\|metal"; then
    echo "   ✓ Found CoreML/Metal framework references"
elif strings "$PLATFORM_BIN_DIR/swictation-daemon" | grep -q -i "coreml"; then
    echo "   ✓ Found CoreML string references in binary"
else
    echo "   ℹ️  CoreML references not found (may be in libonnxruntime.dylib)"
fi
echo ""
...
if "$PLATFORM_BIN_DIR/swictation-daemon" --version > /dev/null 2>&1; then
    ...
    echo "   Binary may have missing dependencies or incorrect architecture"
fi
...
# ============================================================================
# STEP 13: Code Signing Check (Optional but Recommended)
# ============================================================================
echo "🔐 Step 13: Checking code signature..."

if codesign -dv "$PLATFORM_BIN_DIR/swictation-daemon" 2>&1 | grep -q "Signature"; then
    echo "✓ Daemon is code-signed"
    codesign -dv "$PLATFORM_BIN_DIR/swictation-daemon" 2>&1 | head -5 | sed 's/^/   /'
else
    echo "⚠️  Daemon is not code-signed (not required for development)"
    echo "   For distribution, consider signing with: codesign -s 'Developer ID' binary"
fi
echo ""
...
# ============================================================================
echo "✅ macOS ARM64 platform package build complete!"
echo ""
...
if [ -d "$PLATFORM_LIB_DIR" ] && [ "$(ls -A "$PLATFORM_LIB_DIR")" ]; then
    ...
    echo "     (empty - CoreML libraries needed)"
fi
echo ""
echo "📊 Package info:"
echo "   Distribution: $DISTRIBUTION_VERSION"
echo "   Daemon:       $DAEMON_VERSION"
echo "   UI:           $UI_VERSION"
echo "   ONNX Runtime: $ONNX_VERSION (CoreML-enabled)"
echo "   Build date:   $BUILD_DATE"
...
echo "   4. Verify CoreML: Check that Neural Engine is used (Activity Monitor)"
echo "   5. Run in CI/CD: Ready for GitHub Actions macos-14 runners"
echo ""
