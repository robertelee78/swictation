...
fn main() {
    ...
    let frame0_strided: Vec<f32> = (0..encoder_dim)
        .map(|f| features[f * total_time])
        .collect();
    
    println!("\nFrame 0 contiguous (column-major): first 5 = {:?}", &frame0_contiguous[..5]);
    println!("Frame 0 strided (row-major):       first 5 = {:?}", &frame0_strided[..5]);
