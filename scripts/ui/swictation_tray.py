...
def get_socket_path() -> str:
    ...
    if home:
        socket_dir = os.path.join(os.environ.get('XDG_DATA_HOME', os.path.join(home, '.local', 'share')), 'swictation')
...
class SwictationTrayApp(QApplication):
    ...
    def __init__(self, argv):
        ...
        # Setup paths
        self.icon_path = Path(__file__).parent / "swictation_logo.png"
        self.socket_path = get_socket_path()
        print(f"Using socket path: {self.socket_path}")

        # Track Tauri UI process
        self.tauri_process = None
        # Native bundles keep executable siblings under ../bin.
        default_binary = Path(__file__).resolve().parent.parent / "bin" / "swictation-ui"
